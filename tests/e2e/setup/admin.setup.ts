...
test('authenticates the seeded admin user', async ({ page }) => {
    const playwrightE2eEnv = getPlaywrightE2eEnv();

    await page.goto('/login');
    await page.getByTestId('auth-login-email').locator('input').fill(playwrightE2eEnv.adminEmail);
    await page
    ...
    await page.context().storageState({
        path: path.join(process.cwd(), 'tests/.auth/admin.json'),
    });
});

...
export const useThemeMembers = () => {
    ...
    const getCurrentPreset = (permissions: ThemeMemberPermissions): PermissionPreset => {
        for (const [preset, config] of Object.entries(permissionPresets)) {
            ...
            const matches = Object.keys(config.permissions).every(
                (key) =>
    ...
    const searchUsers = async (query: string, themeId: string) => {
        ...
        try {
            const response = (await useApiFetch(
                `/api/users/search?search=${query}&theme_id=${themeId}`,
                {
            ...
            searchResults.value = response.data.users.map((member) => {
                //vérifier que avatar_path est défini, si oui ajouter back/api/media/ devant
                const config = useRuntimeConfig();
                return {
                    ...member,
                    avatar_path: member.avatar_path
                        ? `${config.public.BACKEND_URL}/api/media/${member.avatar_path}`
                        : null,
                } as ThemeMember;
            });
        } catch (error: any) {
    ...
    const updateMemberPermissions = async (
        ...
        try {
            ...
            console.error(error.value);
            throw new Error(
                error.message || 'Erreur lors de la mise à jour des permissions du membre',
            );
        } finally {
            loading.value = false;
        }

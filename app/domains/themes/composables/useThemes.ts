...
export const useThemes = () => {
    ...
    const fetchThemesPage = async (page: number = currentPage.value, options?: { force?: boolean }) => {
        ...
        try {
            const response = await useApiFetch(`/api/themes?page=${page}&per_page=${perPage.value}`, {
            ...
            const pagination: Pagination | undefined =
                root.data?.pagination ?? root.pagination
            ...
            throw new Error(error?.message || `Erreur lors de la récupération des thèmes (page ${page})`)
        } finally {
            loading.value = false
        }
    }
    ...
    const createTheme = async (themeData: { title: string; color: string; playground_id?: string }, options?: {
        targetPage?: number
    }) => {
        loading.value = true

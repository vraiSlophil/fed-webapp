...
const { currentTheme: selectedTheme, loading, updateTheme, deleteTheme, leaveTheme } = useThemes();
...
<template>
    <div>
        ...
        >
            ...
            <div class="flex flex-grow mx-4 min-w-0 cursor-pointer">
                <!-- Mode normal -->
                <div v-if="!isEditing" class="flex items-center gap-3 flex-grow min-w-0">
            ...
            <div :class="isStoredVariant ? 'items-center' : ''">
                <template v-if="isStoredVariant">
                    <slot :text-color="textColor" :theme="theme" name="stored-actions" />
                </template>
                <template v-else>
                    ...
                    <template v-else>
                        <div class="flex flex-row items-center justify-center gap-2">
                            ...
                            <div>
                                ...
                                <Popover ref="colorPopoverRef" target="prev">
                                    <div
                                        class="p-3 flex items-center justify-center gap-2 flex-col"
                                    >
                                        <div class="mb-3">
                                            <ColorPicker v-model="editedColor" inline />
                                        </div>
                                        <div
                                            class="flex items-center justify-center flex-row gap-3"
                                        >
            ...
            <input ref="colorPickerRef" v-model="editedColor" class="hidden" type="color" />
        </div>
        <div
            ...
            data-no-drag
        >
            <TaskList :is-theme-open="isThemeOpen" :theme="theme" class="cursor-auto" />
        </div>
    </div>
    ...
    >
        <div class="confirmation-content flex items-center gap-3 m-4">
            ...
            <span
                ...
                <strong>{{ selectedTheme?.title }}</strong> ?</span
            >
        </div>
        <template #footer>
            <Button label="Non" rounded text @click="deleteDialogVisible = false" />
            <Button
                :loading="loading"
                label="Oui"
                outlined
    ...
    >
        <div class="confirmation-content flex items-center gap-3 m-4">
            <span class="material-symbols-rounded text-yellow-500 text-2xl">warning</span>
            <span
                >Êtes-vous sûr de vouloir quitter le thème
                <strong>{{ theme.title }}</strong> ?</span
            >
        </div>
        <template #footer>
            <Button label="Non" rounded text @click="leaveDialogVisible = false" />
            <Button
                :loading="loading"
                label="Oui"
                outlined

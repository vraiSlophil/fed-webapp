...
const { isOwner, canAddTask } = useThemePermissions(toRef(props, 'theme'));
...
const { stats: themeStats, loading: statsLoading, fetchThemeStats } = useThemeStats();
...
const loadTasksAndStats = async () => {
    await Promise.all([loadTasks(), fetchThemeStats(props.theme.theme_id)]);
};
...
const getCurrentStatusOption = () => {
    return (
        statusOptions.find((option) => option.value === currentStatusFilter.value) ||
        statusOptions[0]
    );
};
...
const getCurrentArchiveOption = () => {
    return (
        archiveOptions.find((option) => option.value === currentArchivedFilter.value) ||
        archiveOptions[0]
    );
};
...
<template>
    <div class="h-full flex flex-col rounded-b-[2.25rem]">
        <div
            class="flex flex-col gap-1 p-4 border-b border-neutral-300 dark:border-neutral-700 transition-all"
        >
            <!-- Barre d'outils -->
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        >
                            <template #option="slotProps">
                                <div class="flex items-center gap-2">
                                    <span
                                        class="material-symbols-rounded text-sm text-neutral-400"
                            ...
                            <template #value="slotProps">
                                <div class="flex items-center gap-2">
                                    <span class="material-symbols-rounded text-sm text-neutral-400">
                                        {{
                                            (currentStatusFilter as any)?.icon ||
                                            statusOptions[0].icon
                                        }}
                                    </span>
                                    <span>
                                        {{
                                            (currentStatusFilter as any)?.label ||
        ...
        <div class="overflow-y-auto max-h-[50vh]">
            ...
            >
                ...
                <div class="space-y-3">
                    ...
                    >
                        <div class="flex items-center gap-2">
                            <span
                                >{{ themeStats.active }} tâche{{
                                    themeStats.active > 1 ? 's' : ''
                                }}
                                active{{ themeStats.active > 1 ? 's' : '' }}</span
                            >
                            <span
                                >{{ themeStats.done }} terminée{{
                                    themeStats.done > 1 ? 's' : ''
                                }}</span
                            >
                            <Tag
                                :style="{
                                    backgroundColor: props.theme.color + '44',
                                    color: textColor,
                                }"
                                severity="secondary"
                            >
                        ...
                        <Button
                            :aria-label="
                                showDetailedStats ? 'Masquer les détails' : 'Afficher les détails'
                            "
                            class="w-10 h-10"
                            outlined
                            rounded
                    ...
                    <div
                        ...
                        class="pt-3 grid grid-cols-2 md:grid-cols-3 gap-3 text-sm"
                    >
                        <!-- Statistiques par statut -->
                        <div class="bg-white/10 dark:bg-white/10 rounded-lg p-3 space-y-2">
                            <h4 class="font-semibold text-neutral-700 dark:text-neutral-300">
                                Par statut
                            </h4>
                            ...
                            <div class="flex items-center justify-between">
                                <span class="flex items-center gap-1">
                                    <span class="material-symbols-rounded text-green-500 text-sm"
                                        >check_circle</span
                                    >
                                    Terminé
                                </span>
                        ...
                        <div class="bg-white/10 dark:bg-neutral/10 rounded-lg p-3 space-y-2">
                            ...
                            <div class="flex items-center justify-between">
                                <span class="flex items-center gap-1">
                                    <span class="material-symbols-rounded text-neutral-500 text-sm"
                                        >archive</span
                                    >
                                    Archivées
                                </span>
                        ...
                        <div
                            class="bg-white/10 dark:bg-neutral/10 rounded-lg p-3 space-y-2 col-span-2 md:col-span-1"
                        >
                            <h4 class="font-semibold text-neutral-700 dark:text-neutral-300">
                                Derniers 7 jours
                            </h4>
            ...
            <!-- Liste des tâches -->
            <div class="flex-1">
                <div v-if="loading" class="flex items-center justify-center w-full h-42">
                ...
                <div
                    ...
                    class="p-8 text-center text-neutral-500 dark:text-neutral-400"
                >
                    ...
                    <p>
                        {{
                            currentArchivedFilter ? 'Aucune tâche archivée' : 'Aucune tâche trouvée'
                        }}
                    </p>
                    <p v-if="!currentArchivedFilter" class="text-sm mt-1">
            ...
            <div
                ...
                class="p-4 border-t border-neutral-300 dark:border-neutral-700"
            >
                <Paginator

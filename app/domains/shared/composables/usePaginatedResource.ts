...
export type PaginatedFetcherOptions<T, TFilters, TResponse> = {
     ...
     */
    parseResponse: (response: TResponse) => { items: T[]; pagination: Pagination }
}
...
>(
    ...
    const filters = ref<TFilters>((initialFilters ? {...(initialFilters as any)} : {}) as TFilters)
    ...
    const loadPage = async (targetPage: number) => {
        ...
        try {
            ...
            const {items: newItems, pagination: newPagination} = parseResponse(response as TResponse)

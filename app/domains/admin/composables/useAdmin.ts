...
export const useAdmin = () => {
    ...
    const fetchUsers = async () => {
        ...
        try {
            ...
            const response = ((await useApiFetch(`/api/admin/users?${params.toString()}`)) as any)
                .data as UserResponse;
            users.value = response.users.map((user) => {
                return {
                    ...user,
                    avatar_path: user.avatar_path
                        ? `${config.public.BACKEND_URL}/api/media/${user.avatar_path}`
                        : null,
                };
            });
    ...
    const fetchUserDetails = async (user: User) => {
        try {
            const response = ((await useApiFetch(`/api/admin/users/${user.user_id}`)) as any)
            ...
            selectedUser.value = (() => {
                ...
                return {
                    ...
                    avatar_path: userData.avatar_path
                        ? `${useRuntimeConfig().public.BACKEND_URL}/api/media/${userData.avatar_path}`
                        : null,
                };
            })();

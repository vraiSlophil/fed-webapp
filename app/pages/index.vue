...
const calculateOpacity = (x: number, y: number): number => {
    ...
    const distance = Math.sqrt(
        normalizedDistanceX * normalizedDistanceX + normalizedDistanceY * normalizedDistanceY,
    );
...
const opacity = computed(() =>
    calculateOpacity(mousePosition.value.x, mousePosition.value.y).toFixed(2),
);
    ...
    <div>
        ...
        >
            <h1 class="!text-7xl font-mono">
                ...
                <TypewriterText
                    :cursor-symbol="'_'"
                    :phrases="// ['créez', 'planifiez', 'partagez' ]
                    ['planifiez', 'partagez']"
                    :type-speed="80"
            ...
            <div class="absolute inset-0 overflow-visible pointer-events-none">
                ...
                >
                    <path :d="animatedPath" fill="none" stroke="currentColor" stroke-width="2" />
                </svg>
            </div>
        </section>

        <section
            class="min-h-screen relative flex flex-col items-center justify-center overflow-x-hidden"
        >
            <!-- Features Section -->
        </section>

...
describe('Playground - CRUD', () => {
    ...
    it('permet de créer un nouveau playground', () => {
        ...
        cy.get("input[placeholder='Slug du playground']").should(
            'have.value',
            uniquePlaygroundSlug,
        );
    ...
    it('permet de modifier un playground existant', () => {
        ...
        cy.get("input[placeholder='Nom du playground']").clear();

        cy.wait(100);

        cy.get("input[placeholder='Nom du playground']").type(editedPlaygroundName);

        cy.get("input[placeholder='Slug du playground']").should(
            'have.value',
            editedPlaygroundSlug,
        );
    ...
    it('charge le playground via son slug', () => {
        ...
        cy.contains('span', editedPlaygroundName)
            .parents('div.flex.justify-between.items-center.flex-col')
            .within(() => {
                cy.get("button[title='Charger le Playground']").should('not.be.disabled').click();
            });
        ...
        cy.location('pathname', { timeout: 10000 }).should(
            'eq',
            `/playground/${editedPlaygroundSlug}`,
        );
        ...
        cy.contains('span', editedPlaygroundName)
            .parents('div.flex.justify-between.items-center.flex-col')
            .within(() => {
                cy.get("button[title='Charger le Playground']").should('be.disabled');
            });
    });
    ...
    it('permet de supprimer un playground', () => {
        ...
        cy.contains('span', editedPlaygroundName)
            .parents('div.flex.justify-between.items-center.flex-col')
            .within(() => {
                cy.get("button[title='Supprimer le Playground']").not('[disabled]').click();
            });
